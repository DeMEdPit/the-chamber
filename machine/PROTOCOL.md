...
| `reset` | | `ok` |
| `input` | `mode`: `keyboard` or `joystick` (what the arrow keys feed); `port`: optional, the port the keys feed as a joystick: 2 (until set: the port the programs of the series read), 1 (the games that read it, Boulder Dash among them), or 0 for both ports at once, one stick in each, which a game reading either port sees | `ok {input, port}` |
