...
// site's files and a stand-in mainnet built from the site's proven copies.
//
// What it holds (every line is PASS or FAIL; the process exits 1 on any FAIL):
//   the handshake states the capabilities the machine has, and the bytes it
//   was handed hashed equal to its pins; the embedded document makes no
//   network request of any kind after its own load, its policy forbids one,
//   and it cannot reach the host's document; the firmware boots to READY and
//   a loaded PRG runs; the bare machine boots and runs the same PRG; every
//   refusal code is produced by exactly the input that should produce it;
//   reads never need LAB, a write does, and the first write is reported as
//   INTERVENED and stays so; tampered machine bytes are refused and nothing
//   runs; a destroyed frame rejects what was pending and a fresh one boots;
//   the standalone build reads the stand-in chain and reaches READY.
//
...
try {
  ...
  check(r.type === 'ready' && r.hashes === 'checked' && r.firmware === true, `ready: hashes ${r.hashes}, firmware ${r.firmware}, ${r.ms} ms`);
  ...
  const typed = await pg.evaluate(() => window.harness.request('type', { text: 'X' }, { timeout: 5000 }));
  check(typed.ok && typed.r.typed === true, 'type resolves when done');

  // 7. a destroyed frame rejects what is pending; a fresh one boots
  ...
  const frames = await pg.evaluate(() => window.harness.frameCount());
  check(frames === 0, `frames left after destroy: ${frames}`);

  // 8. tampered machine bytes are refused and nothing runs
  ...
  // 9. the bare machine boots and runs the same program
  const bare = await pg.evaluate(() => window.harness.boot({ firmware: false }));
  check(!bare.error && bare.ready.type === 'ready' && bare.ready.firmware === false, `bare machine ready, firmware ${(bare.ready || {}).firmware}${bare.error ? ': ' + bare.error.code : ''}`);
  ...
  // 10. the standalone build reads the stand-in chain itself
  ...
  const sst = await sp.evaluate(() => ({ phase: READY64.state.phase, error: READY64.state.error, build: READY64.state.build, fw: READY64.state.firmware, fetched: READY64.state.fetched }));
