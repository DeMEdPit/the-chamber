...
try {
  ...
  check(lines0.now === 'Tony: Born for Adventure (C64 demo) · PINNED · PURE' && lines0.chain === '67 programs · playing Tony: Born for Adventure (C64 demo)' && lines0.file === '.prg · .d64 · .crt' && lines0.controls === 'joystick 2 · AUTO · bare · sound on' && /^running Tony: Born for Adventure \(C64 demo\) · \d+ lines$/.test(lines0.log),
  ...
  const hintWords = await pg.evaluate(() => ({ hints: [...document.querySelectorAll('.ctl .hint')].map((h) => h.innerText.trim()), fine: getComputedStyle(document.querySelector('.ctl .fine-only')).display, account: document.getElementById('about-controls').nextElementSibling.nextElementSibling.textContent }));
  ...
  check(keys.groups === 'PLAY|MACHINE', `THE CONTROLS is grouped: ${keys.groups.replace(/\|/g, ', ')}`);
  ...
  check(/^joystick 1 · AUTO · bare · sound on$/.test((await lines()).controls), `THE CONTROLS' line follows the switch (${(await lines()).controls})`);
  ...
  check((await lines()).controls === 'keyboard · ON · sound on' && (await lines()).now === 'OpenROMs pressing 1 at READY · nothing playing', `the lines under the firmware, READY and nothing playing (${(await lines()).controls} | ${(await lines()).now})`);
  await pg.fill('#search', 'tony');
  await pg.click('#rows .row[data-work="tony"] button.load');
  ...
  check(!!reopened && (await pg.evaluate(() => document.getElementById('input-mode').checkVisibility() && document.querySelector('#bay-controls > .bb').getBoundingClientRect().height > 100 && document.querySelector('#bay-controls > summary').getAttribute('aria-expanded') === 'true')), 'and opens again on its header, the controls back');
  ...
  for (const [w, h] of [[390, 844], [1000, 900], [1440, 900], [1440, 800], [1440, 680]]) {   // a phone; one column wider than the frame; two columns on a tall window; a shorter one, where the log gives up height; a short one, where the machine gives up size and the stage still stays whole
    ...
    const bays0 = await pw.evaluate(() => { const b = window.machinePage.bays; const top = (sel) => document.querySelector(sel).getBoundingClientRect().top; const order = ['#bay-now', '#bay-chain', '#bay-file', '#bay-controls', '#bay-log'].map(top); return { closed: Object.values(b).every((x) => !x.open && !x.element), open: Object.values(b).every((x) => x.open && x.element), aria: [...document.querySelectorAll('details.bay > summary')].map((s) => s.getAttribute('aria-expanded')).join(), ordered: order.every((t, i) => i === 0 || t > order[i - 1]), heights: ['now', 'chain', 'file', 'controls', 'log'].map((k) => Math.round(document.getElementById('bay-' + k).getBoundingClientRect().height)) }; });
    ...
    if (w < 1140) await pw.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'log']) window.machinePage.bay(k, true); });   // opened, instantly, for the shape checks below
  ...
  check(phoneHead.closed && phoneHead.oneLine && phoneHead.cut && phoneHead.trustShown && phoneHead.line === 'Tony: Born for Adventure (C64 demo) · PINNED · PURE' && phoneHead.controls === 'joystick 2 · AUTO · bare · sound on' && phoneHead.state, `on the phone the bays are closed, each line one line tall, the name cut short and the trust words whole (${phoneHead.line})`);
  const rail = await pt.evaluate(() => ['now', 'chain', 'file', 'controls', 'log'].map((k) => { const p = document.getElementById('bay-' + k).getBoundingClientRect(), l = document.getElementById('sum-' + k).getBoundingClientRect(), m = document.querySelector('#bay-' + k + ' .bm').getBoundingClientRect(); return { gap: Math.round(m.left - l.right), inset: Math.round(p.right - m.right), centred: Math.abs((p.right - 16 - m.right) - (m.left - (l.right + 12))) }; }));
  ...
  await pt.evaluate(() => window.machinePage.bay('controls', true));
  ...
  await pt.evaluate(() => { window.machinePage.bay('controls', false); location.hash = '#how-auto'; });
  const revealed = await until(() => pt.evaluate(() => { const k = window.machinePage.bays.controls, h = document.getElementById('how-auto'); return k.open && k.element && h.open && h.classList.contains('is-open') ? true : null; }), 3000, 50);
  ...
  await pm.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'log']) window.machinePage.bay(k, false); document.getElementById('bay-now').scrollIntoView({ block: 'center', behavior: 'instant' }); Object.assign(window.machinePage.readout.tune, { wait: 10 }); window.machinePage.readout.replay('now'); });
