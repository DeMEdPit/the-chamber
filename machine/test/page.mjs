...
try {
  ...
  check(await pg.evaluate(() => Object.entries(window.machinePage.bays).every(([k, b]) => (k === 'ports' ? !b.open && !b.element : b.open && b.element) && !b.moving) && [...document.querySelectorAll('details.bay > summary')].every((s) => s.getAttribute('aria-expanded') === (s.parentElement.id === 'bay-ports' ? 'false' : 'true')) && document.querySelectorAll('details.bay').length === 6), 'on a wide screen six bays: five start open and THE PORTS closed, each header saying so');
  ...
  for (const [w, h] of [[390, 844], [1000, 900], [1440, 900], [1440, 800], [1440, 680]]) {   // a phone; one column wider than the frame; two columns on a tall window; a shorter one, where the log gives up height; a short one, where the machine gives up size and the stage still stays whole
    ...
    const bays0 = await pw.evaluate(() => { const b = window.machinePage.bays; const top = (sel) => document.querySelector(sel).getBoundingClientRect().top; const order = ['#bay-now', '#bay-chain', '#bay-file', '#bay-controls', '#bay-ports', '#bay-log'].map(top); return { closed: Object.values(b).every((x) => !x.open && !x.element), open: Object.entries(b).every(([k, x]) => (k === 'ports' ? !x.open && !x.element : x.open && x.element)), aria: [...document.querySelectorAll('details.bay > summary')].map((s) => s.getAttribute('aria-expanded')).join(), ordered: order.every((t, i) => i === 0 || t > order[i - 1]), heights: ['now', 'chain', 'file', 'controls', 'ports', 'log'].map((k) => Math.round(document.getElementById('bay-' + k).getBoundingClientRect().height)) }; });
    if (w < 1140) check(bays0.closed && bays0.aria === 'false,false,false,false,false,false' && bays0.ordered && bays0.heights.every((x) => x > 40 && x < 80), `at ${w} wide: the six bays start closed, two lines each (${bays0.heights.join(', ')}px), NOW PLAYING first under the pad, then the list, the file door, the controls, the ports and the log`);
    else check(bays0.open && bays0.aria === 'true,true,true,true,true,false', `at ${w} wide: five bays start open and THE PORTS closed`);
    if (w < 1140) await pw.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'ports', 'log']) window.machinePage.bay(k, true); });   // opened, instantly, for the shape checks below
  ...
  const rail = await pt.evaluate(() => ['now', 'chain', 'file', 'controls', 'ports', 'log'].map((k) => { const p = document.getElementById('bay-' + k).getBoundingClientRect(), l = document.getElementById('sum-' + k).getBoundingClientRect(), m = document.querySelector('#bay-' + k + ' .bm').getBoundingClientRect(); return { gap: Math.round(m.left - l.right), inset: Math.round(p.right - m.right), centred: Math.abs((p.right - 16 - m.right) - (m.left - (l.right + 12))) }; }));
  ...
  await pt.evaluate(() => { window.machinePage.bay('controls', true); window.machinePage.bay('ports', true); });
  ...
  const hintLines = await pt.evaluate(() => [...document.querySelectorAll('.ctl .hint')].filter((h) => h.getClientRects().length).map((h) => ({ t: h.innerText.trim().slice(0, 30), h: Math.round(h.getBoundingClientRect().height) })));
  check(hintLines.length >= 14 && hintLines.every((h) => h.h < 24), `on the phone every hint under a control and a port is one line tall (${hintLines.map((h) => h.h).join(', ')}px)`);
  ...
  await pm.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'ports', 'log']) window.machinePage.bay(k, false); document.getElementById('bay-now').scrollIntoView({ block: 'center', behavior: 'instant' }); Object.assign(window.machinePage.readout.tune, { wait: 10 }); window.machinePage.readout.replay('now'); });
