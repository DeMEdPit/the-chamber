...
try {
  ...
  check(keys.groups === 'PLAY|MACHINE|VIEW', `THE KEYS is grouped: ${keys.groups.replace(/\|/g, ', ')}`);
  ...
  check(!!reset, 'RESET');
  // FULL SCREEN, presentation only: the screen goes full, the player at the largest whole multiple of the picture that fits
  // with the badge's room, the badge hung from its corner, EXIT the way back, nothing rebuilt. Escape is the browser's own
  // way out and a synthetic key cannot press it, so EXIT is what the gate presses.
  await pg.fill('#search', 'tony');
  await pg.click('#rows .row[data-work="tony"] button.load');
  await until(() => pg.evaluate(() => document.getElementById('state').dataset.phase === 'running'), 30000, 500);
  const corner = await pg.evaluate(() => { const r = (id) => document.getElementById(id).getBoundingClientRect(); const f = r('frame'), c = r('full-corner'); window.machinePage.machine.gateMark = 'before full screen'; return { shown: getComputedStyle(document.getElementById('full-corner')).display !== 'none', hung: Math.abs(c.top - f.bottom) < 3 && f.right - c.right >= 0 && f.right - c.right < 40, exitHidden: getComputedStyle(document.getElementById('exit-full')).display === 'none', view: [...document.querySelectorAll('.keys .grp')].map((g) => g.textContent).join('|'), enabled: window.machinePage.fullscreen.enabled && !document.getElementById('full').disabled }; });
  check(corner.shown && corner.hung && corner.exitHidden && corner.view === 'PLAY|MACHINE|VIEW' && corner.enabled, `FULL SCREEN hangs from the frame's other corner, EXIT hidden, VIEW the third group of THE KEYS (${corner.view})`);
  await pg.click('#full');
  const full = await until(() => pg.evaluate(() => (window.machinePage.fullscreen.active && window.machinePage.fullscreen.scale > 0 ? (() => { const r = (id) => document.getElementById(id).getBoundingClientRect(); const f = r('frame'), l = r('link'), e = r('exit-full'); return { scale: window.machinePage.fullscreen.scale, frame: `${Math.round(f.width)}x${Math.round(f.height)}`, centred: Math.abs((f.left + f.right) / 2 - innerWidth / 2) < 2, link: Math.abs(l.top - f.bottom) < 3 && l.bottom <= innerHeight, exit: getComputedStyle(document.getElementById('exit-full')).display === 'block' && e.top >= 0 && e.right <= innerWidth, corner: getComputedStyle(document.getElementById('full-corner')).display === 'none', state: document.getElementById('state').dataset.phase, focused: !!document.activeElement && document.activeElement.tagName === 'IFRAME', logged: /full screen: the machine at 3 times its picture, 1152 by 816/.test(document.getElementById('log').textContent) }; })() : null)), 10000, 100);
  check(!!full && full.scale === 3 && full.frame === '1152x816' && full.centred && full.link && full.exit && full.corner && full.state === 'running' && full.focused && full.logged, `FULL SCREEN at 1180x900: the machine at 3 times its picture (${full && full.frame}), centred, the badge hung from its corner, EXIT shown, the machine running and holding the keys`);
  await pg.click('#exit-full');
  const back = await until(() => pg.evaluate(() => (!window.machinePage.fullscreen.active && window.machinePage.fullscreen.scale === 0 ? { frame: Math.round(document.getElementById('frame').getBoundingClientRect().width), fw: document.getElementById('player').style.getPropertyValue('--fw'), state: document.getElementById('state').dataset.phase, same: window.machinePage.machine.alive && window.machinePage.machine.gateMark === 'before full screen', logged: /full screen left/.test(document.getElementById('log').textContent) } : null)), 10000, 100);
  check(!!back && back.frame === 768 && back.fw === '' && back.state === 'running' && back.same && back.logged, `EXIT FULL SCREEN: the frame back at 768, the same machine still running, nothing rebuilt`);
  ...
  const pad = await pt.evaluate(() => { const r = (sel) => document.querySelector(sel).getBoundingClientRect(); const f = r('#frame'), ring = r('#ring'), fire = r('#touch .fire'), chain = r('.chain'), link = r('#link'); return { above: ring.top - f.bottom, below: chain.top - ring.bottom, level: Math.abs((ring.top + ring.bottom) / 2 - (fire.top + fire.bottom) / 2), badgeClear: fire.top - link.bottom, corner: getComputedStyle(document.getElementById('full-corner')).display, view: getComputedStyle(document.querySelector('.keys .fine-only')).display }; });
  check(Math.abs(pad.above - 22) < 1 && Math.abs(pad.below - 22) < 1 && pad.level < 1 && pad.badgeClear > 0 && pad.corner === 'none' && pad.view === 'none', `the pad has the same air above and below the ring (${Math.round(pad.above)} and ${Math.round(pad.below)}), FIRE level with it, the badge clear of FIRE; no FULL SCREEN on a touch screen`);
