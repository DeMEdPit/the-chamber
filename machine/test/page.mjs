...
try {
  ...
  const motion = await pg.evaluate(() => { const m = new DOMMatrixReadOnly(getComputedStyle(document.querySelector('#bay-keys .bm'), '::after').transform); return { body: getComputedStyle(document.querySelector('#bay-keys > .bb')).transitionDuration, mark: getComputedStyle(document.querySelector('#bay-keys .bm'), '::after').transitionDuration, marker: getComputedStyle(document.querySelector('#bay-keys > summary')).listStyleType, fallen: Math.abs(m.a) < 0.01 && Math.abs(m.b - 1) < 0.01 }; });
  check(/^0\.24s/.test(motion.body) && motion.mark === '0.32s' && motion.marker === 'none' && motion.fallen, `the body opens in 240ms and the mark's upright falls a quarter turn onto the bar in 320ms; open, it lies on the bar (${motion.body}; ${motion.mark})`);
  ...
  await until(() => pg.evaluate(() => { const m = new DOMMatrixReadOnly(getComputedStyle(document.querySelector('#bay-keys .bm'), '::after').transform); return Math.abs(m.a - 1) < 0.01 && Math.abs(m.b) < 0.01 ? true : null; }), 1500, 30);   // the upright rises for 320 ms, a little after the drawer has closed
