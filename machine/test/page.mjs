...
try {
  ...
  // THE INSTRUMENTS: the card drawn from instruments.json, PURE for a program with no instruments of its own (the Tony
  // demo), the switches gated; INSTRUMENTS by the control lays the sound's two over the picture in the free bands and
  // names them in NOW PLAYING and the provenance; PURE again empties the layer; the chain strip's switch hides the badge alone
  const instJson = JSON.parse(await (await fetch(`${A.base}/machine/instruments.json`)).text());
  const inst0 = await pg.evaluate(() => ({ rows: [...document.querySelectorAll('#bay-instruments .irow')].map((r) => r.dataset.inst), groups: [...document.querySelectorAll('#bay-instruments .grp')].map((g) => g.textContent), mode: window.machinePage.instruments.mode, chosen: window.machinePage.instruments.chosen, line: window.machinePage.bays.instruments.line,
    gated: [...document.querySelectorAll('#bay-instruments .irow:not([data-host]) .isw')].every((b) => b.disabled), layer: document.querySelectorAll('#layer .ipanel').length, modeRow: (document.getElementById('now').textContent.match(/MODE\s*([^\n]*)/) || [])[1] || '', badge: !document.getElementById('link').hidden, chainOn: window.machinePage.instruments.on.chain }));
  check(inst0.rows.join() === instJson.instruments.map((i) => i.id).join() && inst0.groups.join('|') === instJson.groups.map((g) => g.label).join('|') && inst0.mode === 'pure' && !inst0.chosen && inst0.line === 'off · PURE' && inst0.gated && inst0.layer === 0 && /^PURE · nothing on this page reaches into the machine/.test(inst0.modeRow) && inst0.badge && inst0.chainOn,
        `THE INSTRUMENTS is drawn from instruments.json (${inst0.rows.join(', ')}) and a program with none of its own starts PURE: the switches gated, nothing over the picture, the badge shown (${inst0.line})`);
  await pg.evaluate(() => window.machinePage.bay('instruments', true));
  await pg.click('#mode button[data-mode="instruments"]');
  const inst1 = await until(() => pg.evaluate(() => { const i = window.machinePage.instruments; return i.mode === 'instruments' && i.layers.length === 2 && document.querySelectorAll('#layer .ipanel').length === 2 ? { ...i, line: window.machinePage.bays.instruments.line, modeRow: (document.getElementById('now').textContent.match(/MODE\s*([^\n]*)/) || [])[1] || '', prov: JSON.parse(document.getElementById('provenance-json').value) } : null; }), 5000, 100);
  check(!!inst1 && inst1.chosen && inst1.layers.join() === 'scope,spectrum' && inst1.panels.scope.band === 'top-left' && inst1.panels.spectrum.band === 'top-right' && inst1.line === 'scope · spectrum · INSTRUMENTS' && /^INSTRUMENTS · scope, spectrum · reads only, nothing written/.test(inst1.modeRow) && inst1.prov.mode === 'INSTRUMENTS' && inst1.prov.layers.join() === 'scope,spectrum',
  ...
  await pg.mouse.move(dragFrom.x, dragFrom.y); await pg.mouse.down(); await pg.mouse.move(dragFrom.x + 160, dragFrom.y + 120, { steps: 8 }); await pg.mouse.up();
  const dragged = await pg.evaluate(() => { const p = window.machinePage.instruments.panels.scope; return { band: p.band, left: p.left, top: p.top }; });
  check(dragged.band === 'dragged' && dragged.left > 100 && dragged.top > 60, `a panel drags by its title bar and stays where it is put (${dragged.left}, ${dragged.top})`);
  await pg.click('#layer .ipanel[data-inst="scope"] .ifold');
  const pill = await pg.evaluate(() => window.machinePage.instruments.panels.scope);
  check(pill && pill.pill && pill.height < 30, `the mark folds a panel to a pill on its title bar (${JSON.stringify(pill)})`);
  await pg.click('#inst-chain .isw');
  const strip = await pg.evaluate(() => ({ hidden: document.getElementById('link').hidden, prov: JSON.parse(document.getElementById('provenance-json').value), nodes: window.machinePage.nodes }));
  check(strip.hidden && strip.prov.nodes && strip.prov.node && strip.prov.mode === 'INSTRUMENTS', 'THE CHAIN STRIP\'s switch hides the badge alone: the provenance still names the node and the reads');
  await pg.click('#inst-chain .isw');
  await pg.click('#mode button[data-mode="pure"]');
  const inst2 = await pg.evaluate(() => ({ ...window.machinePage.instruments, layer: document.querySelectorAll('#layer .ipanel').length, gated: [...document.querySelectorAll('#bay-instruments .irow:not([data-host]) .isw')].every((b) => b.disabled), line: window.machinePage.bays.instruments.line, prov: JSON.parse(document.getElementById('provenance-json').value).mode, badge: !document.getElementById('link').hidden }));
  check(inst2.mode === 'pure' && inst2.layer === 0 && inst2.gated && inst2.on.scope && inst2.line === 'off · PURE' && inst2.prov === 'PURE' && inst2.badge, 'PURE again: the layer empty, the switches gated but keeping their positions, the provenance PURE, the badge back');
  await pg.evaluate(() => window.machinePage.bay('instruments', false));
  const portsJson = JSON.parse(await (await fetch(`${A.base}/machine/ports.json`)).text());
  const portsDom = await pg.evaluate(() => ({ labels: [...document.querySelectorAll('#bay-ports .prt dt')].map((d) => d.textContent), ids: [...document.querySelectorAll('#bay-ports .prt .live')].map((e) => e.id.slice(5)), hints: [...document.querySelectorAll('#bay-ports .prt .hint')].map((h) => h.textContent), groups: [...document.querySelectorAll('#bay-ports .grp')].map((g) => g.textContent).join('|'),
    switches: !!document.querySelector('#bay-ports #input-mode') && !!document.querySelector('#bay-ports #firmware') && !!document.querySelector('#bay-ports #how-auto') && !document.querySelector('#bay-controls select:not(#ways)'), controlsGroups: document.querySelectorAll('#bay-controls .grp').length, rows: window.machinePage.ports, open: window.machinePage.bays.ports.open }));
  const byId = Object.fromEntries(portsJson.ports.map((x) => [x.id, x]));
  const want = { labels: [], hints: [] };
  for (const g of portsJson.groups) { for (const id of g.ports) { want.labels.push(byId[id].label); want.hints.push(byId[id].hint); } if (g.control) { want.labels.push(g.control.toUpperCase()); want.hints.push(g.control === 'input' ? 'what the arrows feed; AUTO chooses from the file' : null); } }
  check(portsDom.labels.join('|') === want.labels.join('|') && portsDom.ids.join() === portsJson.groups.flatMap((g) => g.ports).join() && portsDom.hints.join('|') === want.hints.filter((h) => h !== null).join('|') && portsDom.groups === portsJson.groups.map((g) => g.label).join('|') && portsDom.switches && portsDom.controlsGroups === 0,
        `THE PORTS is drawn from ports.json: ${portsDom.labels.length} rows in ${portsJson.groups.length} groups (${portsDom.groups.replace(/\|/g, ', ')}), INPUT and FIRMWARE in the groups of the sockets they govern, THE CONTROLS one list`);
  check(!portsDom.open && JSON.stringify(portsDom.rows) === JSON.stringify({ port1: 'empty', port2: 'your stick', cartridge: 'empty', serial: 'no drive', cassette: 'nothing', user: 'nothing', av: 'the screen · sound on', sockets: 'empty · the machine runs bare' }),
        `THE PORTS starts closed on a wide screen and its rows say what is in each socket (${Object.values(portsDom.rows).join(' | ')})`);
  check(await pg.evaluate(() => Object.entries(window.machinePage.bays).every(([k, b]) => (k === 'ports' || k === 'instruments' ? !b.open && !b.element : b.open && b.element) && !b.moving) && [...document.querySelectorAll('details.bay > summary')].every((s) => s.getAttribute('aria-expanded') === (s.parentElement.id === 'bay-ports' || s.parentElement.id === 'bay-instruments' ? 'false' : 'true')) && document.querySelectorAll('details.bay').length === 7), 'on a wide screen seven bays: five start open, THE PORTS and THE INSTRUMENTS closed, each header saying so');
  ...
  // the work's own instrument: the canary's page keeps SID OUTPUT at the top right; loaded on a page whose mode nobody chose,
  // it comes on there by itself, and a fresh visit of the Tony demo is PURE
  {
    const pc = await b.newPage({ viewport: { width: 1180, height: 900 } });
    await pc.goto(`${A.base}/machine/`, { waitUntil: 'load' });
    await until(() => pc.evaluate(() => document.getElementById('state').dataset.phase === 'running'), 90000, 500);
    check(await pc.evaluate(() => window.machinePage.instruments.mode === 'pure' && !window.machinePage.instruments.chosen), 'a fresh visit on the Tony demo is PURE, nobody having chosen');
    await pc.fill('#search', 'head mind');
    await pc.click('#rows .row[data-work="perception-canary"] button.load');
    const own = await until(() => pc.evaluate(() => { const i = window.machinePage.instruments; return i.mode === 'instruments' && i.panels.scope ? { ...i, line: window.machinePage.bays.instruments.line, modeRow: (document.getElementById('now').textContent.match(/MODE\s*([^\n]*)/) || [])[1] || '', where: document.getElementById('inst-scope-where').textContent } : null; }), 60000, 200);
    check(!!own && !own.chosen && own.own.scope === 'top-right' && own.panels.scope.band === 'top-right' && own.layers.join() === 'scope' && own.line === 'scope · INSTRUMENTS' && /^INSTRUMENTS · scope · reads only/.test(own.modeRow) && /the work's own · top right/.test(own.where),
  ...
  check((await pg.evaluate(() => window.machinePage.ports.serial)) === 'no drive · games.d64 is read by the page instead', `THE PORTS says the bus has no drive and the page reads the disk instead (${await pg.evaluate(() => window.machinePage.ports.serial)})`);
  ...
  check(await pg.evaluate(() => /silent if the phone is/.test(document.getElementById('sound').parentElement.textContent) && /click the machine to give it your keys/.test(document.querySelector('.ctl').textContent)), 'THE CONTROLS carries the sound and keyboard facts the sentence used to');
  const hintWords = await pg.evaluate(() => ({ hints: [...document.querySelectorAll('.controls .hint')].map((h) => h.innerText.trim()), fine: getComputedStyle(document.querySelector('.ctl .fine-only')).display, account: document.getElementById('about-controls').nextElementSibling.nextElementSibling.textContent }));
  check(hintWords.hints.join('|') === "other keys type; its port is under THE PORTS|one direction suits the series; both for eight ways|Escape is RUN/STOP · Home is CLR/HOME · F1 to F7|starts on your first click or key|the machine starts over; READY under the firmware" && hintWords.fine === 'inline' && /Escape is RUN\/STOP and never leaves this page/.test(hintWords.account) && /takes both ports when it cannot tell/.test(hintWords.account),
        `the hints are one short line each and the edge cases live in the account below (${hintWords.hints.length} hints)`);
  const keys = await pg.evaluate(() => ({ groups: [...document.querySelectorAll('.controls .grp')].map((g) => g.textContent).join('|'), open: document.getElementById('how-auto').open, summary: document.querySelector('#how-auto summary').textContent, target: document.querySelector('#how-auto a').getAttribute('href'), targetExists: !!document.getElementById('about-controls'),
    diag: getComputedStyle(document.querySelector('dt.touch-only')).display, silent: getComputedStyle(document.querySelector('span.touch-only')).display, hints: [...document.querySelectorAll('.ctl dd')].map((d) => d.querySelectorAll('.hint').length <= 1).every(Boolean),
    lede: document.querySelector('.lede').textContent, subheads: [...document.querySelectorAll('.about h3')].map((h) => h.id).join() }));
  check(keys.groups === '', 'THE CONTROLS is one list of the hands, the switches gone to the sockets they govern');
  ...
  await pg.evaluate(() => window.machinePage.bay('ports', true));   // the switches sit in THE PORTS, closed by default
  ...
  const motion = await pg.evaluate(() => { const bm = document.querySelector('#bay-controls .bm'); const m = new DOMMatrixReadOnly(getComputedStyle(bm, '::after').transform), w = new DOMMatrixReadOnly(getComputedStyle(bm).transform); const body = getComputedStyle(document.querySelector('#bay-controls > .bb')); return { body: body.transitionDuration, bodyEase: (/^[^,(]*(?:\([^)]*\))?/.exec(body.transitionTimingFunction) || [''])[0].trim(), mark: getComputedStyle(bm, '::after').transitionDuration, markEase: getComputedStyle(bm, '::after').transitionTimingFunction, spin: getComputedStyle(bm).transitionDuration, marker: getComputedStyle(document.querySelector('#bay-controls > summary')).listStyleType, fallen: Math.abs(m.a) < 0.01 && Math.abs(m.b - 1) < 0.01, turned: Math.abs(w.a + 1) < 0.01 && Math.abs(w.b) < 0.01 }; });
  check(/^0\.24s/.test(motion.body) && motion.mark === '0.24s' && motion.spin === '0.24s' && motion.markEase === motion.bodyEase && motion.marker === 'none' && motion.fallen && motion.turned, `the drawer, the mark's half turn and its upright's fall share one clock and one curve (${motion.body}, ${motion.bodyEase}); open, the mark has turned and the upright lies on the bar`);
  const controlsLineBefore = (await lines()).controls;
  await pg.click('#bay-controls > summary');
  const closing = await pg.evaluate(() => { const b = window.machinePage.bays.controls; return { moving: b.moving, element: b.element, open: b.open }; });
  const closed = await until(() => pg.evaluate(() => { const b = window.machinePage.bays.controls; return !b.open && !b.element && !b.moving ? true : null; }), 3000, 50);
  ...
  for (const [w, h] of [[390, 844], [1000, 900], [1440, 900], [1440, 800], [1440, 680]]) {   // a phone; one column wider than the frame; two columns on a tall window; a shorter one, where the log gives up height; a short one, where the machine gives up size and the stage still stays whole
    ...
    const bays0 = await pw.evaluate(() => { const b = window.machinePage.bays; const top = (sel) => document.querySelector(sel).getBoundingClientRect().top; const order = ['#bay-now', '#bay-chain', '#bay-file', '#bay-controls', '#bay-ports', '#bay-instruments', '#bay-log'].map(top); return { closed: Object.values(b).every((x) => !x.open && !x.element), open: Object.entries(b).every(([k, x]) => (k === 'ports' || k === 'instruments' ? !x.open && !x.element : x.open && x.element)), aria: [...document.querySelectorAll('details.bay > summary')].map((s) => s.getAttribute('aria-expanded')).join(), ordered: order.every((t, i) => i === 0 || t > order[i - 1]), heights: ['now', 'chain', 'file', 'controls', 'ports', 'instruments', 'log'].map((k) => Math.round(document.getElementById('bay-' + k).getBoundingClientRect().height)) }; });
    if (w < 1140) check(bays0.closed && bays0.aria === 'false,false,false,false,false,false,false' && bays0.ordered && bays0.heights.every((x) => x > 40 && x < 80), `at ${w} wide: the seven bays start closed, two lines each (${bays0.heights.join(', ')}px), NOW PLAYING first under the pad, then the list, the file door, the controls, the ports, the instruments and the log`);
    else check(bays0.open && bays0.aria === 'true,true,true,true,true,false,false', `at ${w} wide: five bays start open, THE PORTS and THE INSTRUMENTS closed`);
    if (w < 1140) await pw.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'ports', 'instruments', 'log']) window.machinePage.bay(k, true); });   // opened, instantly, for the shape checks below
    ...
    check(!lay.hint, `at ${w} wide: no sentence under the machine; its facts are in THE CONTROLS`);
  ...
  const phoneHead = await pt.evaluate(() => { const b = window.machinePage.bays; const s = document.getElementById('sum-now').getBoundingClientRect(), e = document.querySelector('#sum-now .e'), last = document.querySelector('#sum-now .f:last-child').getBoundingClientRect(); return { closed: Object.values(b).every((x) => !x.open && !x.element), oneLine: s.height < 24, cut: e.scrollWidth > e.clientWidth, trustShown: last.right <= s.right + 0.5 && last.width > 60, line: b.now.line, controls: b.controls.line, state: document.getElementById('state').getBoundingClientRect().right <= document.querySelector('#bay-now > summary').getBoundingClientRect().right }; });
  check(phoneHead.closed && phoneHead.oneLine && phoneHead.cut && phoneHead.trustShown && phoneHead.line === 'Tony: Born for Adventure (C64 demo) · PINNED · PURE' && phoneHead.controls === 'joystick 2 · sound on' && phoneHead.state, `on the phone the bays are closed, each line one line tall, the name cut short and the trust words whole (${phoneHead.line})`);
  const rail = await pt.evaluate(() => ['now', 'chain', 'file', 'controls', 'ports', 'instruments', 'log'].map((k) => { const p = document.getElementById('bay-' + k).getBoundingClientRect(), l = document.getElementById('sum-' + k).getBoundingClientRect(), m = document.querySelector('#bay-' + k + ' .bm').getBoundingClientRect(); return { gap: Math.round(m.left - l.right), inset: Math.round(p.right - m.right), centred: Math.abs((p.right - 16 - m.right) - (m.left - (l.right + 12))) }; }));
  ...
  await pt.evaluate(() => { window.machinePage.bay('controls', true); window.machinePage.bay('ports', true); window.machinePage.bay('instruments', true); });
  check(await pt.evaluate(() => getComputedStyle(document.querySelector('dt.touch-only')).display === 'block' && getComputedStyle(document.querySelector('dd.touch-only')).display === 'block' && getComputedStyle(document.querySelector('span.touch-only')).display === 'inline' && getComputedStyle(document.querySelector('.ctl .fine-only')).display === 'none'), 'on a coarse pointer DIAGONALS and the phone note show in THE CONTROLS, the fine pointer\'s sound note not');
  const hintLines = await pt.evaluate(() => [...document.querySelectorAll('.ctl .hint')].filter((h) => h.checkVisibility()).map((h) => ({ t: h.innerText.trim().slice(0, 30), h: Math.round(h.getBoundingClientRect().height) })));
  check(hintLines.length >= 15 && hintLines.every((h) => h.h < 24), `on the phone every hint under a control, a port and the mode is one line tall (${hintLines.map((h) => h.h + (h.h >= 24 ? ' ' + h.t : '')).join(', ')}px)`);
  ...
  await pt.evaluate(() => { window.machinePage.bay('ports', false); location.hash = '#how-auto'; });
  const revealed = await until(() => pt.evaluate(() => { const k = window.machinePage.bays.ports, h = document.getElementById('how-auto'); return k.open && k.element && h.open && h.classList.contains('is-open') ? true : null; }), 3000, 50);
  ...
  const still = await pm.evaluate(() => { const d = document.getElementById('bay-controls'); const before = getComputedStyle(d.querySelector(':scope > .bb')).transitionDuration; d.querySelector(':scope > summary').click(); const b = window.machinePage.bays.controls; return { before, moving: b.moving, element: b.element, open: b.open }; });
  ...
  await pm.evaluate(() => { for (const k of ['now', 'chain', 'file', 'controls', 'ports', 'instruments', 'log']) window.machinePage.bay(k, false); document.getElementById('bay-now').scrollIntoView({ block: 'center', behavior: 'instant' }); Object.assign(window.machinePage.readout.tune, { wait: 10 }); window.machinePage.readout.replay('now'); });
