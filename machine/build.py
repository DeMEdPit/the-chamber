...
.panel.bay{{padding:0;--bay-ms:240ms;--bay-ease:cubic-bezier(.2,.8,.2,1)}}   /* one clock for the drawer and the mark, so they can never drift apart (the owner, 2026-09-23: the mark must match the section's own speed) */
...
.bm{{grid-area:mark;position:relative;width:12px;height:12px;align-self:center;justify-self:center;transition:transform var(--bay-ms) var(--bay-ease)}}
...
.bm::after{{left:5px;top:0;width:2px;height:12px;transform-origin:50% 50%;transition:transform var(--bay-ms) var(--bay-ease)}}
/* as the card opens the whole mark turns half a turn while its upright falls a quarter turn onto the bar, so a plus becomes a
   minus in one movement on the drawer's own clock and curve; closing runs it back (the owner's ask, 2026-09-23, after a site
   whose plus falls into a minus) */
.bay.is-open>summary .bm{{transform:rotate(180deg)}}
.bay.is-open>summary .bm::after{{transform:rotate(90deg)}}
.bb{{display:grid;grid-template-rows:0fr;opacity:0;transition:grid-template-rows var(--bay-ms) var(--bay-ease),opacity 180ms ease}}
...
@media(prefers-reduced-motion:reduce){{.bb,.bm,.bm::before,.bm::after{{transition:none}}}}
...
.fold{{font-size:.78rem;line-height:1.45;color:var(--muted);--bay-ms:240ms;--bay-ease:cubic-bezier(.2,.8,.2,1)}}
...
/* a group's heading, in NOW PLAYING and THE CONTROLS alike: the word and a rule to the edge */
...
.ctl{{margin:0;font-size:.85rem;color:var(--ink);line-height:1.5}}
/* every control has the same anatomy: the label, then the control or the legend, then one muted line */
.ctl dt{{font:700 .6rem/1.9 {MONO};letter-spacing:.16em;color:var(--muted);margin-top:8px}}
.ctl dd{{margin:0}}
.ctl .hint{{display:block;margin:3px 0 0;font-size:.78rem;line-height:1.45;color:var(--muted)}}
.ctl .why{{display:block;font:600 .66rem/1.6 {MONO};letter-spacing:.06em;color:var(--accent2);margin:4px 0 0}}
.ctl .how{{margin:4px 0 0}}
...
/* one column: the two wrappers dissolve and the panels take the phone's order, NOW PLAYING first under the pad (what is
   ...
   file door, the controls and the log (LEAVE THE MACHINE went on 2026-09-23, the owner's word: the kicker at the top and the
