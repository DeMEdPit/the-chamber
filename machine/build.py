...
@media(min-width:1140px) and (min-height:820px){{.stage{{position:sticky;top:14px}}}}
@media(min-width:1140px) and (min-height:820px) and (max-height:899px){{.log{{height:100px}}}}
  ...
  <aside class="column">
    ...
    <section class="panel keys" aria-labelledby="lab-keys">
      ...
      <dl class="keys">
        ...
        <dt>DIAGONALS</dt><dd><select class="mode" id="ways" aria-label="how the ring reads a diagonal" autocomplete="off"><option value="4" selected>one direction</option><option value="8">both directions, eight ways</option></select> <span class="hint">on a touch screen the ring is read by angle, and the hole is rest. The programs of the series stand still when two directions are pressed together, so a diagonal is read as one direction, left or right first, until you say otherwise.</span></dd>
        <dt>KEYBOARD</dt><dd>click the machine to give it your keys; they are the C64's. Escape is RUN/STOP and never leaves this page, Home is CLR/HOME, the function keys are F1 to F7.</dd>
        <dt>SOUND</dt><dd><button type="button" class="b" id="sound">SOUND ON</button> <span class="hint">the page plays what the machine's sound chip makes; it starts with your first tap or key on this page, and a phone on silent stays silent</span></dd>
        <dt>FIRMWARE</dt><dd><select class="mode" id="firmware" aria-label="the firmware" autocomplete="off"><option value="auto" selected>auto: as the program needs</option><option value="off">off: bare, as on chain</option><option value="on">on: OpenROMs pressing 1, READY first</option></select> <span class="why" id="firmware-why">AUTO · decides when a program loads</span> <span class="hint">the programs of the series run bare, as they do on chain, and run the same with the firmware present, because they bank it out as they start. AUTO reads a file of yours for what it needs: a program that calls the KERNAL or BASIC, hooks the KERNAL's interrupt vectors, is BASIC itself or has no stub a bare machine can start it by gets the on-chain OpenROMs and READY first; one that needs none of that runs bare. The scan reads byte patterns, and a program can hide a dependency from it, so the switch stays yours: on boots to READY and RESET brings the prompt back; off is bare.</span></dd>
