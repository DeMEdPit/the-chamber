...
<div class="machine">
  ...
  <aside class="column">
    ...
    <section class="panel" aria-labelledby="lab-file">
      ...
      <p class="fine">It stays in this browser and is sent nowhere. The page checks its shape, a load address and a size that fits, and claims nothing else about it: NOW PLAYING says YOUR FILE. A program that wants BASIC or the KERNAL needs FIRMWARE on. The disk and cartridge doors, and a switch that reads a file for what it needs, come in later phases.</p>
    </section>
    ...
    <section class="panel" aria-labelledby="lab-keys">
      ...
      <dl class="keys">
        ...
        <dt>FIRMWARE</dt><dd><select class="mode" id="firmware" aria-label="the firmware"><option value="off" selected>off: bare, as on chain</option><option value="on">on: OpenROMs pressing 1, READY first</option></select> <span class="hint">the programs of the series run bare, as they do on chain, and run the same with the firmware present, because they bank it out as they start. With it on, the machine boots the on-chain OpenROMs to READY, RESET brings the prompt back, and the keyboard is yours.</span></dd>
...
<section class="about">
  ...
  <p>The machine document is GPL-2.0-only, nopsta's licence; the page around it is MIT; <a href="LICENSES.md">what is what</a>. A program file of your own, a .prg, runs here too, through FROM A FILE: it stays in this browser, the page checks only its shape, and NOW PLAYING says YOUR FILE. A disk image, a cartridge and a switch that reads a file for what it needs come in later phases.</p>
</section>"""
