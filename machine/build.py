...
.lede{{margin:0 0 18px;color:var(--muted);font-weight:450;line-height:1.4;font-size:clamp(.95rem,2.6vw,1.05rem);max-width:768px}}   /* wraps under the title at the machine's width */
...
.keys dd{{margin:0}}
.keys .why{{display:block;font:600 .66rem/1.6 {MONO};letter-spacing:.06em;color:var(--accent2);margin:4px 0 2px}}
...
@media(max-width:1139px){{.machine{{grid-template-columns:1fr;row-gap:14px}}.stage,.column{{display:contents}}.screen{{max-width:768px;order:1}}.touch{{order:2;margin:0}}.chain{{order:3}}.file{{order:4}}.now{{order:5}}.logbox{{order:6;margin:0}}.keys{{order:7}}.leave{{order:8}}}}
...
<p class="lede"><b>READY 64</b>: the Commodore 64 emulator nopsta stored on Ethereum in 2022, read off the chain and held to its pins before it runs.</p>
<div class="machine">
  <div class="stage">
    <div class="screen">
  ...
  <aside class="column">
    ...
    <section class="panel keys" aria-labelledby="lab-keys">
      ...
      <dl class="keys">
        <dt>INPUT</dt><dd><select class="mode" id="input-mode" aria-label="what the arrow keys feed" autocomplete="off"><option value="joystick" selected>joystick in port 2</option><option value="keyboard">the keyboard</option></select></dd>
        <dt>JOYSTICK</dt><dd>arrows move; Z, X or space is FIRE. The programs of the series read port 2.</dd>
        <dt>DIAGONALS</dt><dd><select class="mode" id="ways" aria-label="how the ring reads a diagonal" autocomplete="off"><option value="4" selected>one direction</option><option value="8">both directions, eight ways</option></select> <span class="hint">how the touch ring reads a diagonal; the programs of the series stand still when two directions are pressed together</span></dd>
        <dt>KEYBOARD</dt><dd>click the machine to give it your keys. Escape is RUN/STOP and never leaves this page; Home is CLR/HOME; F1 to F7 are the function keys.</dd>
        <dt>SOUND</dt><dd><button type="button" class="b" id="sound">SOUND ON</button> <span class="hint">starts with your first tap or key on this page; a phone on silent stays silent</span></dd>
        <dt>FIRMWARE</dt><dd><select class="mode" id="firmware" aria-label="the firmware" autocomplete="off"><option value="auto" selected>auto: as the program needs</option><option value="off">off: bare, as on chain</option><option value="on">on: OpenROMs pressing 1, READY first</option></select> <span class="why" id="firmware-why">AUTO · decides when a program loads</span> <span class="hint">AUTO reads a file for what it needs and says why here; on boots the on-chain OpenROMs to READY; off is bare, as on chain. The scan reads byte patterns, so the switch stays yours.</span></dd>
...
<section class="about">
  ...
  <p>A program is read from its contract the way anyone can read it: <a href="https://etherscan.io/address/{chamber}#readContract" target="_blank" rel="noopener">the Chamber's</a> <code>prg(id)</code>, stamped with the block you load it at, is held to the pinned base outside its 42-byte stamp and to the row and the block inside it; <a href="https://etherscan.io/address/{perception}#readContract" target="_blank" rel="noopener">the Perception Chamber Canary's</a> <code>prgWithBrain(1)</code> is held to the frozen program outside its mind and to the head revision's record inside it; the two older tokens are held to their pinned hashes. The words on the provenance line mean what they say: <b>PINNED</b>, the bytes matched a commitment this page held before it asked; <b>CONTRACT-CONSISTENT</b>, they matched what the same node reported in the same session; <b>NODE-REPORTED</b>, a fact one node stated; <b>YOUR FILE</b>, a file of yours, about which the page claims nothing. One node is asked, and it is named. The pins are in <a href="catalogue.json">the catalogue</a>, which <a href="verify.py">a public script</a> checks against the chain for anyone who runs it (<a href="CATALOGUE.md">how</a>).</p>
  ...
  <p>The firmware is a switch. The programs of the series run bare, as they do on chain, and run the same with the firmware present, because they bank it out as they start. AUTO reads a file of yours for what it needs: a program that calls the KERNAL or BASIC, hooks the KERNAL's interrupt vectors, is BASIC itself or has no stub a bare machine can start it by gets the on-chain OpenROMs and READY first; one that needs none of that runs bare, and a program that banks the ROMs out and sets its own interrupt vectors is read as bare whatever its data looks like. The scan reads byte patterns, and a program can hide a dependency from it, so the switch stays yours: on boots to READY and RESET brings the prompt back; off is bare. On a touch screen the direction ring is read by angle and the hole is rest; the programs of the series stand still when two directions are pressed together, so a diagonal is read as one direction, left or right first, unless DIAGONALS says both.</p>
  <p>The machine document is GPL-2.0-only, nopsta's licence; the page around it is MIT; <a href="LICENSES.md">what is what</a>. A program file of your own, a .prg, runs here too, through FROM A FILE: it stays in this browser, the page checks only its shape and reads it for what it needs, and NOW PLAYING says YOUR FILE. With FIRMWARE on AUTO, a program that calls the KERNAL or BASIC gets the on-chain OpenROMs and one that does not runs bare, the reason said under the switch. A disk image and a cartridge come in later phases.</p>
</section>"""
