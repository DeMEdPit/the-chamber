...
.lede{{margin:0 0 18px;color:var(--muted);font-weight:450;line-height:1.4;font-size:clamp(.95rem,2.6vw,1.05rem);max-width:768px}}   /* a block under the title as wide as the player, wrapping at the gap between the columns, not a band across the page */
...
<div class="machine">
  ...
  <aside class="column">
    ...
    <section class="panel keys" aria-labelledby="lab-keys">
      ...
      <p class="grp">PLAY</p>
      <dl class="keys">
        <dt>INPUT</dt><dd><select class="mode" id="input-mode" aria-label="what the arrow keys feed" autocomplete="off"><option value="joystick" selected>joystick in port 2</option><option value="keyboard">the keyboard</option></select> <span class="hint">what the arrows and letters feed; AUTO sets it for each program</span></dd>
        <dt>JOYSTICK</dt><dd>arrows move · Z, X or space is FIRE <span class="hint">the programs of the series read port 2</span></dd>
        <dt class="touch-only">DIAGONALS</dt><dd class="touch-only"><select class="mode" id="ways" aria-label="how the ring reads a diagonal" autocomplete="off"><option value="4" selected>one direction</option><option value="8">both directions, eight ways</option></select> <span class="hint">read by angle, the hole is rest; the programs of the series stand still with two directions held, so a diagonal counts as one unless you say both</span></dd>
        <dt>KEYBOARD</dt><dd>click the machine to give it your keys <span class="hint">Escape is RUN/STOP and never leaves this page · Home is CLR/HOME · F1 to F7 are the function keys</span></dd>
        <dt>SOUND</dt><dd><button type="button" class="b" id="sound">SOUND ON</button> <span class="hint">starts with your first tap or key on this page<span class="touch-only"> · a phone on silent stays silent</span></span></dd>
      </dl>
