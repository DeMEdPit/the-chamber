...
| `index.html` | built: the page, THE MACHINE at `/machine/`: the frame, NOW PLAYING, FROM THE CHAIN, FROM A FILE, the keys, the log; every panel is a bay whose header carries one live line (what is true now, in the page's own words), open on a wide screen and closed on a phone; its policy allows scripts and frames from this site only and connections to this site and the shared endpoints only, nothing inline | MIT |
