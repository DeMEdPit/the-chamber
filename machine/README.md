...
| `index.html` | built: the page, THE MACHINE at `/machine/`: the frame, NOW PLAYING, FROM THE CHAIN, FROM A FILE, the controls, the ports, the instruments, the log; every panel is a bay whose header carries one live line (what is true now, in the page's own words), open on a wide screen and closed on a phone; its policy allows scripts and frames from this site only and connections to this site and the shared endpoints only, nothing inline | MIT |
...
| `readout.js` | a line cut short in a bay's header slides to show its end and back, on a phone, for a closed bay in view, never under reduced motion; a cut of sixteen pixels or less stays still under the fade, a leg never takes less than 1.2 s, and a line that changes while moving glides home and starts again; one switch in host.js turns it off, one word picks the shape (bounce, or once) | MIT |
| `audio.js` | the page plays the machine's sound: samples pulled over the port and scheduled on the page's audio clock, paced as nopsta's player paces them; the clock asked for back when the page comes into view after a phone's browser stopped it, the clock itself watched (a context that says it runs while its time stands still is stalled, and the next tap builds a fresh clock rather than asking the old one; on an iPhone, whose Safari can run a context silently after an interruption, the first tap after any return builds one), the schedule re-synced, and no stale buffer offered as sounding while it is stopped or stalled | MIT |
...
| `instruments.js` | THE INSTRUMENTS, the rack: the two modes (PURE, nothing on this page reads or reaches into the machine; INSTRUMENTS, read-only layers named in NOW PLAYING and the provenance), each instrument's switch, the panels laid over the machine's picture at the places a work's own page keeps them or the next free band, draggable and folded to pills, drawn from the sound the page already plays; the default follows the work; the panels over the picture the token's own chassis; COLOUR, GREEN (the site's green on the token's chassis) or SCENE (the token's colour rule: ground, ink and body from the picture, read from the machine document's `colours` once a second while a scope is on); `INSTRUMENTS.md` | MIT |
| `instruments/` | the instruments themselves and what they share: `scope.js` (in the card, the SID's output as a trace with a stated window, a trigger with hysteresis, a fractional stride and a held scale; over the picture, the token's own wave, the whole buffer at raw amplitude) and `spectrum.js` (the same sound by frequency: the whole buffer, a Hann window, log bands, a slow reference, decibels, ballistics at draw time), each reading only the buffer sounding now and never the machine; `chassis.js`, the token's chassis in the picture's pixels; `romfont.js`, the titles from the character ROM; `scene.js`, the token's colour rule and its adoption; `colour.js`, the colour arithmetic (a hex parsed, its luminance, a paler tint, a mix, OKLab both ways with gamut mapping, the ladder a meter climbs in even steps of lightness, the token's shade) | MIT |
| `instruments.json` | the instruments the page carries, in their groups by what each reads, what each needs of the machine and requires of the program, its sizes; the build draws the card from it; another machine is another description | data |
| `ports.json` | the machine's sockets as this build has them (the two control ports, the cartridge port, the serial bus with no drive behind it, the cassette and user ports with nothing, video and sound, the ROM sockets), in the groups THE PORTS draws them in; the build renders the card from it and the page writes what it has in each socket now; another machine is another description | data |
...
| `test/` | the headless gate (`bridge.mjs`), its server with a stand-in mainnet built from the copies (`serve.mjs`), the harness page; the scan's gate (`scan.mjs`, Node alone); the instruments' gate (`instruments.mjs`, Node alone: the token's colour rule and its adoption, the token's wave, the titles from the ROM, the chassis layout); the disk image reader's gate (`d64.mjs`) and its image maker (`make-d64.mjs`); the cartridge reader's gate (`crt.mjs`) and its maker with a 46-byte probe cartridge of our own (`make-crt.mjs`); the verifier's gate (`test_verify.py`) and its stand-in node (`mocknode.py`) | MIT |
...
node machine/test/instruments.mjs  # the instruments' rules, ported from the token's page, in Node alone
