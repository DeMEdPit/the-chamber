...
| `instruments.js` | THE INSTRUMENTS, the rack: the two modes (PURE, nothing on this page reads or reaches into the machine; INSTRUMENTS, read-only layers named in NOW PLAYING and the provenance), each instrument's switch, the panels laid over the machine's picture at the places a work's own page keeps them or the next free band, draggable and folded to pills, drawn from the sound the page already plays; the default follows the work; COLOUR, GREEN or SCENE, the picture's own colour read from the machine document (`colours`) once a second while a scope is on; `INSTRUMENTS.md` | MIT |
| `instruments/` | the instruments themselves, one module each: `scope.js` (the SID's output as a trace: a stated window, a trigger with hysteresis, a fractional stride, a held scale) and `spectrum.js` (the same sound by frequency: the whole buffer, a Hann window, log bands, a slow reference, decibels, ballistics at draw time); each reads only the buffer sounding now and never the machine; `colour.js`, the colour arithmetic they share (a hex parsed, its luminance, a paler tint) | MIT |
